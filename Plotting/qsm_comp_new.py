...
# def get_original_distances( new_algo=False ):
#     if not new_algo:
#         with open( os.path.join('data', 'labeled', 'offset', 'qsm_set_full.json'), 'r' ) as f:
#             trees = json.load(f)
#     else:
#         proj_dir = os.path.join('data', 'testing', 'qsm_subset', 'projected_new', 'original')
#         trees = [os.path.join(proj_dir, f) for f in os.listdir(proj_dir) if f.endswith(".npy")]

#     distances = []
#     # print(f"Number of original processed clouds: {len(trees)}")
#     # print(f"Paths: {trees}")
#     for tree in trees:
#         cloud = np.load(tree)
...
# def get_projected_distances( input_dir, denoised=False, new_algo=False ):
...
#     tree_paths = [os.path.join(input_dir, f) for f in os.listdir(input_dir) if f.endswith(suffix)]

#     distances = []
#     nan_count_total = 0
#     row_count_total = 0

#     # print(f"Number of predicted processed clouds: {len(tree_paths)}")
#     # print(f"Paths: {tree_paths}")
...
#             print(f"WARNING: File not found: {tree_path}")
#             continue

#         cloud = np.load(tree_path)
...
# def plot_full_qsm_comparison(dist_orig, dist_pred, mean_dists, errors, improvements, imp_errors, model_labels, new_algo, plot_save_path=None):
...
#     if new_algo:
#         main_title = f"Comparison of Pipeline QSM to TreeQSM"
#     else:
#         main_title = f"Comparison of Pipeline QSM to Sphere Following"
#     fig.suptitle(main_title, fontsize=plt.rcParams['figure.titlesize'])
...
#     distances_orig = get_original_distances(new_algo)
...
#         else:
#             input_dir_new = os.path.join('data', 'testing', 'qsm_subset', 'projected_new', model)
...
#     else:
#         plot_save_path = os.path.join('plots', 'PipelineEval', 'new_comp_testset.png')

#     plot_full_qsm_comparison(distances_orig, dist_new, mean_dists, errors, improvements, imp_errors, model_labels, new_algo, plot_save_path)
...
# =============================================================================
if __name__ == "__main__":
    testset_mode = False  # <<<< USER SETTING: True for new QSMs, False for old dataset
    ...
    if testset_mode:
        ...
        for model_name in models:
            ...
            mean, err = compute_mean_distance_and_error(current_model_distances_agg)
            ...
            if len(orig_pointwise) > 0 and len(orig_pointwise) == len(model_pointwise):
                ...
                imp, imp_err = compute_mean_distance_and_error(improvement_diffs)
            else:
