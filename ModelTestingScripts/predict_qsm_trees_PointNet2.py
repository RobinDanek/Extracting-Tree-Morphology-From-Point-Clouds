...
def makePredictions(offset_model, noise_model):
    ...
    raster_plot_loader_list = [raster_plot_3_loader, raster_plot_4_loader, raster_plot_6_loader, raster_plot_8_loader]

    # make predictions and store cloud + offset
    for plot, loader, raster_loader in zip( plot_list, plot_loader_list, raster_plot_loader_list ):
        model_offset = model_dict[f"O_P{plot}"].cuda()
        model_noise = model_dict[f"N_P{plot}"].cuda()

        for tree, raster_tree in progress_bar(zip(loader, raster_loader), master=None, total=len(raster_loader)):
            ...
            # Classify noise
            with torch.no_grad():
                noise_output = model_noise.forward_hierarchical_streaming(raster_tree, return_loss=False)
            ...
            with torch.no_grad():
                offset_output = model_offset.forward_hierarchical_streaming(raster_tree, return_loss=False)
