...
def index_points(points, idx):
    ...
    """
    assert torch.all((idx >= 0) & (idx < points.shape[1])), f"Invalid index found in idx! Max: {idx.max()}, N: {points.shape[1]}"
...
def farthest_point_sample(xyz, npoint):
    ...
    assert torch.all((centroids >= 0) & (centroids < N)), f"Invalid index in centroids! Max: {centroids.max()}, N: {N}"
    return centroids
